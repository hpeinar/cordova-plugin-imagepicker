 ...
 */
 ...
 var ImagePicker = {
	...
	validateOutputType: function(options) {
	...
	hasReadPermission: function(callback) {
	  return cordova.exec(callback, null, "ImagePicker", "hasReadPermission", []);
	},
	
	requestReadPermission: function(callback, failureCallback) {
	...
	*/
	getPictures: function(success, fail, options) {
		if (!options) {
			options = {};
		}
		...
		const sourceType = options.sourceType ? options.sourceType : this.SourceType.PHOTOLIBRARY;
		...
		const mediaType = options.mediaType ? options.mediaType : this.MediaType.PICTURE;
